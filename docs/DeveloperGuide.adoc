...
The `find` command was enhanced to allow a more rigorous search. The new find command allows users to search for Persons in the humanDEX using fields such as Name, Email, Phone, and Address.
It supports partial search and users are required to input fields using prefixes.

==== Implementation

The enhanced `find` command was implemented by first, modifying the parsing mechanism in `FindCommandParser`. After checking whether any of the prefixes exist in the `argMultiMap`, the parser uses four methods to parse each field. An example of the method is as follows: +
...
These parsed arguments are delivered to the `PersonContainsKeywordsPredicate` (`Predicate`) by creating a `FindFields` class within the `PersonContainsKeywordsPredicate` class. The `FindFields` class uses four lists to store the search inputs for each of the `Name`, `Email`, `Phone`, and `Address` fields, as such: +
...
The class also provides one 'setter' method and two 'getter' methods for each of the fields. The former 'getter' method returns a Optional, while the latter returns a Stream, for various uses. The latter was implemented so that the string of `getNameKeywords().map(List::stream).orElseGet(Stream::empty)` commands do not have to be repeated multiple times. Below is an example of the two 'getter' methods for `Name`: +
...
After a successful parsing, the execution of the `find` command can be continued. To return partial matches for the search command, a different `match` mechanism was used. Instead of using `StringUtil.containsWordIgnoreCase` to check for matches, the new code uses a simple `String.toLowerCase().contains(String.toLowerCase())` method to search for partial matches as well. Below is a line extracted from `PersonContainsKeywordPredicate.java`. +
...
Custom fields is implemented to give the users more control over the data they are keeping about their contacts.
...
The `list` command allow user to list the contacts by tag(s).
...
**Alternative 2:** Use `find` command for finding tags +
**Pros:** Easier to implement as `find` function is already available. +
**Cons:** If contact's details contains same word as tag, contact will be listed. For example if May mail is mayfriend@hotmail.com but does not contain tag "friend", May will be found even though she does not inclulde tag "friend".

=== Sort

The `sort` command allows users to sort their contacts alphabetically by name. It is an undoable command.

=== Implementation

Upon executing the command, the `model` executes `sortFilteredPersonList`, which sorts the AddressBook according to name. +

To enable the sort, a `ReadOnlyPersonComparator` is defined with an overridden `compare` method. +

The `SortCommand` will be enhanced further in the coming week to support sorting by other fields as well as a combination of multiple fields (e.g. sort by name and phone number).
...
Our target user is insurance agents that has lots of contacts. They may need different address book for their different types of clients. If the insurance agents plans on retiring and wants to past the contacts to his/her fellow colleagues, he/she will be able to export the address book and the colleague will be able to import it.
