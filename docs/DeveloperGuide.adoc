...
These parsed arguments are delivered to the `PersonContainsKeywordsPredicate` (`Predicate`) by creating a `FindFields` class within the `PersonContainsKeywordsPredicate` class. For each searchable field, the FindFields class uses a list to store the search inputs. +
...
After a successful parsing, the execution of the `find` command can be continued. The `find` command now returns partial matches. +

The diagram below shows how the enhanced `find` command works. +
...
As of v1.4, passwords can be of any string. This feature will be improved to check for stronger passwords and encrypting the saved password such that no one can just open the
text file and reading the password. Until a better way of storing local passwords are found, the password will exist as a non-hidden file.
...
No password recovery is supported. If the password is forgotted during development, go to the data folder and delete the `pass` file.
