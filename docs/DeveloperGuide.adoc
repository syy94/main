...
**Cons:** If contact's details contains same word as tag, contact will be listed. For example if May's mail is mayfriend@example.com but does not contain tag "friend", May will be found even though she does not include tag "friend".
...
Upon executing the command, the `model` executes `sortFilteredPersonList`, which sorts the AddressBook according to the given field. +

To enable the sort, a `ReadOnlyPersonComparator` is defined with an overridden `compare` method, that holds a static `Prefix` variable to compare two `ReadOnlyPerson` objects by. +
